...
/*   Updated: 2018/02/28 03:23:14 by dmitriy1         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	ft_count_coord_x(t_params* params, double matrix[3][3])
{
	int	y;
	int	x;
	int	z;
...
t_params	**ft_rotate_x(t_params** params, int way)
{
	...
	while (params[y])
	{
		...
		{
			ft_count_coord_x(&params[y][x], matrix);
			x++;
		}
		x = 0;
		y++;
